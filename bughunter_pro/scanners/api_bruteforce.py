...
class APIBruteforcer:
    ...
    def scan(self) -> List[Dict]:
        ...
        logger.info(
            f"[API] Starting API endpoint brute-force on {self.base_url}"
        )
        logger.info(f"[API] Wordlist: {self.config.api_wordlist}")
        ...
        if not words:
            ...
            fallback_path = os.path.join(
                self.config.wordlist_dir, "fallback", "api_endpoints.txt"
            )
            words = load_wordlist(fallback_path)

        if not words:
            logger.error("[API] No wordlist available. Skipping.")
            return self.findings

        logger.info(f"[API] Loaded {len(words)} endpoints to test")
    ...
    def _classify_severity(
        self, status_code: int, path: str, auth_required: bool
    ) -> str:
