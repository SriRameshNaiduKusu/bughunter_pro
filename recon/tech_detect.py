"""
BugHunter Pro - Technology Detection Module

Fingerprints technologies based on HTTP headers, meta tags,
script sources, cookies, and response body patterns.
"""
...
# Signature database: (category, name, detection_patterns)
TECH_SIGNATURES = [
    # Web servers
...
class TechDetector:
    ...
    def detect(self, url: Optional[str] = None) -> Dict[str, List[str]]:
        ...
        # Extra: detect version from headers
        ...
        logger.info(f"[TECH] Detection complete. Found {sum(len(v) for v in self.results.values())} technologies.")
        return self.results