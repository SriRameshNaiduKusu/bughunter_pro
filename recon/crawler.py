"""
BugHunter Pro - Web Crawler Module

Crawls the target website, collects links, forms, parameters, and JS files.
"""
...
from core.utils import make_request, normalize_url, is_same_domain
...
class WebCrawler:
    ...
    def _crawl_recursive(self, url: str, depth: int) -> None:
        ...
        # Check <link> tags, <img> tags, etc. for additional URLs
        for tag in soup.find_all(["link", "img", "source", "video", "audio"], src=True):
            src_url = normalize_url(tag.get("src") or tag.get("href"), url)
            if src_url:
